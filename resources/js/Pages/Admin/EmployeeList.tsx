...
const EmployeeList: React.FC = (): React.ReactNode => {
    ...
    useEffect(() => {
        setLoading(true); // Establece el estado de carga en true al inicio

        // Llamadas API para obtener la lista de empleados, puestos y compañías
        Promise.all([
            ...
            .then(([employeeResponse, positionsResponse, companiesResponse]) => {
                ...
                setLoading(false); // Solo se quita el loading después de que todas las llamadas han finalizado
            })
            .catch((error) => {
                ...
                {employees.map(employee => {
                    ...
                    return (
                        <tr className={'hover'} key={employee.id}>
                            <td className="border px-4 py-2">{employee.first_name + ' ' + employee.last_name_1}</td>
